...
@patch("litellm.completion")
def test_concept_map_xss_mermaid_rejection(mock_completion: MagicMock) -> None:
    """
    Tests that POST /api/v1/analyze returns the correct error response structure
    when the LLM output contains a potential XSS vector (e.g. a <script> tag).
    """
    client = TestClient(app)

    # Configure the mock response with a script tag
    mock_choice = MagicMock()
    ...
    assert data["code"] == "INVALID_LLM_OUTPUT"
    assert data["action"] == "concept_map"
    assert "Rilevato potenziale contenuto XSS non sicuro" in data["message"]


@patch("litellm.completion")
def test_concept_map_empty_mermaid_rejection(mock_completion: MagicMock) -> None:
    """
    Tests that POST /api/v1/analyze returns the correct error response structure
    when the LLM output is empty.
    """
    client = TestClient(app)
